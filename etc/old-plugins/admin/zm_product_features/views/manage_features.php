    ...
    if ('zm_product_features_admin' == ZMRequest::instance()->getParameter('fkt')) {
      switch (ZMRequest::instance()->getParameter('action')) {
        case 'remove_feature':
          ...
          // reload
          ZMRequest::instance()->redirect($request->url('', $zm_nav_params, true));
          break;
    ...
    <tbody>
      <?php foreach ($features as $feature) { $type = $feature->getType(); if (!$type) continue; ?>
        <tr>
          ...
          <td>
            <a class="btn" href="<?php echo $request->url(null, 'action=edit_feature&featureId='.$feature->getId()) ?>">Edit</a>
            <a class="btn del" href="<?php echo $request->url(null, 'action=remove_feature&featureId='.$feature->getId()) ?>" onclick="return zm_user_confirm('Delete feature \'<?php echo addslashes($feature->getName())?>\' ?');">Delete</a>
          </td>
        </tr>
      <?php } ?>
    </tbody>
          ...
          <?php foreach ($productFeatures as $feature) { ?>
            <?php foreach ($feature->getValues() as $index => $value) { ?>
              <tr>
                ...
                <td>
                  <a class="btn" href="<?php echo $request->url(null, 'action=edit_feature_value&featureId='.$feature->getId().'&index='.$index) ?>">Edit</a>
                  <a class="btn del" href="<?php echo $request->url(null, 'action=remove_feature_value&featureId='.$feature->getId().'&index='.$index) ?>" onclick="return zm_user_confirm('Delete feature value \'<?php echo addslashes($value) ?>\' ?');">Delete</a>
                </td>
              </tr>
            <?php } ?>
          <?php } ?>
