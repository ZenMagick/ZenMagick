...
<form action="<?php echo $admin->url() ?>" method="POST">
  <h2><?php _vzm('EZPage Manager') ?> (
      ...
      <a href="<?php echo $admin->url('ezpage', 'editId=0&languageId='.$selectedLanguageId) ?>">Create new</a>
  </h2>
</form>
...
<table class="grid">
  ...
  <?php foreach ($resultList->getResults() as $ezPage) { ?>
    <tr>
      ...
      <td><a href="<?php echo $admin->url('ezpage', 'editId='.$ezPage->getId().'&languageId='.$selectedLanguageId) ?>"><?php echo $html->encode($ezPage->getTitle()) ?></a><td>
      ...
      <td>
        <form action="<?php echo $admin->url('ezpage') ?>" method="POST" onsubmit="return ZenMagick.confirm('<?php _vzm('Delete page id:#%s?', $ezPage->getId()) ?>', this);">
          ...
          <a class="<?php echo $buttonClasses ?>" href="<?php echo $admin->url('ezpage', 'editId='.$ezPage->getId().'&languageId='.$selectedLanguageId) ?>">Edit</a>
        </form>
      </td>
    </tr>
  <?php } ?>
