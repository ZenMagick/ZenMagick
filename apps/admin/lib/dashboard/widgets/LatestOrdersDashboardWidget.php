...
class LatestOrdersDashboardWidget extends DashboardWidget {
    ...
    public function getContents($request) {
        ...
        foreach ($this->container->get('orderService')->getAllOrders($language->getId(), 5) as $order) {
            ...
            $contents .= '    <td><a href="'.$admin->url('orders', 'action=edit&oID='.$order->getId()).'">'.$order->getId().'</a></td>';
            $contents .= '    <td><a href="'.$admin->url('customers', 'action=edit&cID='.$order->getAccountId()).'">'.$name.'</a></td>';
            $contents .= '    <td>'.$this->container->get('localeService')->getLocale()->dateShort($order->getOrderDate()).'</td>';
