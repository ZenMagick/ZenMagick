...
class ZMNoThemeSupportPatch extends ZMFilePatch {
    ...
    function patch($force=false) {
        ...
        $PATCHLINE = "if (!ZMSettings::get('isEnableZMThemes', true)) { \$request = ZMRequest::instance(); \$event = new zenmagick\\base\\events\\Event(null, array('request' => \$request, 'content' => ob_get_clean())); zenmagick\\base\\Runtime::getEventDispatcher()->dispatch('finalise_content', \$event); echo \$event->get('content'); \$request->getSession()->clearMessages(); zenmagick\\base\\Runtime::getEventDispatcher()->dispatch('all_done', new zenmagick\\base\\events\\Event(null, array('request' => \$request))); } /* added by ZenMagick installation patcher */";
