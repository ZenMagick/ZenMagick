...
<?php foreach ($faq_topics as $key => $title) { ?>
    <li><a href="<?php echo $view['router']->generate('gv_faq', array('topic' => $key)) ?>"><?php _vzm($title) ?></a></li>
<?php } ?>
</ul>
...
<?php echo $form->open('gv_redeem', '', true, array('id'=>'gv_redeem')) ?>
  <fieldset>
    ...
    <div>
      ...
      <input type="text" id="gvCode" name="couponCode" value="<?php echo $view->escape($gvRedeem->getCouponCode()) ?>" />
    </div>
  </fieldset>
