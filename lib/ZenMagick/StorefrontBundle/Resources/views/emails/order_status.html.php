...
<?php if (ZenMagick\StoreBundle\Entity\Account::REGISTERED == $currentAccount->getType()) {
    $href = '<a href="'.$net->generate('account_history_info', array('order_id', $currentOrder->getId()), true).'">'.sprintf(_zm("order #%s"), $currentOrder->getId()).'</a>';
} else {
    $href = '<a href="'.$net->generate('guest_history', array(), true).'">'.sprintf(_zm("order #%s"), $currentOrder->getId()).'</a>';
} ?>
