...
class ToolboxMacro extends ToolboxTool {
    ...
    public function categoryTree($categories, $showProductCount=false, $useCategoryPage=false, $activeParent=false, $root=true, $path=null) {
        ...
        foreach ($categories as $category) {
            ...
            echo '<a' . ('' != $class ? ' class="'.$class.'"' : '') . $onclick . ' href="' .
                        $net->url('category', 'cPath='.implode('_', $category->getPath())) .
    ...
    protected function productUploadAttribute($product, $attribute, $uploadIndex) {
        ...
        foreach ($attribute->getValues() as $value) {
            ...
            $text .= '<input type="hidden" name="'.$settingsService->get('uploadOptionPrefix').$uploadIndex.'" value="'.$attribute->getId().'"'.$slash.'>';
            $text .= '<input type="hidden" name="'.$settingsService->get('textOptionPrefix').$settingsService->get('uploadOptionPrefix').$uploadIndex.'" value=""'.$slash.'>';
            array_push($elements, $text);
        }
        $element['html'] = $elements;
        return $element;
