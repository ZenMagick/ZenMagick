...
<?php if (1 == count($reviews)) {
    ...
    $pimg = zm_pimage($review->getProductImage(), $review->getProductName());
    $rimg = zm_image('stars_'.$review->getRating().'.gif', zm_l10n_get("%s of 5 stars!", $review->getRating()));
    ?>
    <h3><a href="<?php zm_href(FILENAME_REVIEWS) ?>"><?php zm_l10n("[More]") ?></a><?php zm_l10n("Reviews") ?></h3>
    <div id="sb_reviews" class="box">
        <p><a href="<?php echo $href ?>"><?php echo $pimg ?></a></p>
