...
<?php if (0 != ZMRequest::getProductId() && isset($zm_product)) { ?>
    ...
    <?php } else if ($isSubscribed) { ?>
        <h3><?php zm_l10n("Notifications") ?></h3>
        <div id="sb_product_notifications" class="box">
            <a href="<?php $net->url(null, 'action=notify_remove') ?>"><img src="<?php $zm_theme->themeURL("images/big_remove.gif") ?>" alt="<?php zm_l10n("Remove product notification") ?>" title="<?php zm_l10n("Remove product notification") ?>" /><br /><?php zm_l10n("Do not notify me of updates to <strong>%s</strong>", $zm_product->getName())?></a>
        </div>
    <?php } else if ($isSubscribed) { ?>
    <?php } ?>
<?php } ?>
