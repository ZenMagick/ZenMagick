...
<?php echo $form->open(FILENAME_CHECKOUT_SUCCESS, 'action=update', true, array('onsubmit'=>null)) ?>
    ...
    <?php if ($request->getAccount()->isRegistered()) { ?>
        <?php $account = '<a href="' . $request->url(FILENAME_ACCOUNT) . '">' . zm_l10n_get("My Account") . '</a>'; ?>
        <p><?php zm_l10n("You can view your full order history by going to the %s page and by clicking on view all orders.", $account) ?></p>
    <?php } ?>
    <?php $customercare = '<a href="' . $request->url(FILENAME_CONTACT_US) . '">' . zm_l10n_get("Customer Service") . '</a>'; ?>
    ...
    <?php if (0 < $voucherBalance) { ?>
        <fieldset>
            ...
            <p>
                ...
                <?php $email = '<a href="' . $request->url(FILENAME_GV_SEND) . '">' . zm_l10n_get("email") . '</a>'; ?>
    ...
    <?php if ($request->isGuest()) { ?>
        <fieldset>
            ...
            <p>
                <?php $lookupLink = '<a href="' . $request->url('guest_history') . '">' . zm_l10n_get("order status check") . '</a>'; ?>
