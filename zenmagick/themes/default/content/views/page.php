...
<?php $toc = ZMEZPages::instance()->getPagesForChapterId($zm_request->getParameter("chapter")); ?>
<?php if (1 < count($toc)) { ?>
    <div id="eztoc">
        <h4><?php zm_l10n("Table of Contents") ?></h4>
        <ul>
        <?php foreach ($toc as $page) { $active = $page->getId() == $zm_page->getId() ? ' class="act"' : ''; ?>
            <li<?php echo $active ?>><a href="<?php zm_ezpage_href($page) ?>"><?php echo $page->getTitle() ?></a></li>
        <?php } ?>
        </ul>
        <?php
            // find pref next
...
<?php } ?>
<h2><?php echo $zm_page->getTitle() ?></h2>
