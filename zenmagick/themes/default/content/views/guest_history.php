...
<?php zm_secure_form(ZM_FILENAME_GUEST_HISTORY, "", 'guest_history', 'post', 'return validate(this);') ?>
  <fieldset>
    ...
    <div>
      ...
      <input type="text" id="email" name="email" value="<?php echo $zm_request->getParameter('email', '') ?>" /> 
    </div>
    <div>
      <label for="orderId"><?php zm_l10n("Order Number") ?></label>
      <input type="text" id="orderId" name="orderId" value="<?php echo $zm_request->getParameter('orderId', '') ?>" /> 
    </div>
  </fieldset>
