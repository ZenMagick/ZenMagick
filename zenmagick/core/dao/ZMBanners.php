...
class ZMBanners {
    ...
    global $zm_request;
        ...
        if ($zm_request->isSecure()) {
            $filter = $this->db_->bindVars(" and banners_on_ssl= :true ", ":true", 1, "integer");
        }
        ...
        if (0 < count($identifierList)) {
            ...
            foreach ($identifierList as $identifier) {
                if (!$first) $filter .= " or ";
                $filter .= $this->db_->bindVars("banners_group = :identifier", ":identifier", $identifier, "string");
                $first = false;
            }
            $filter .= ")";
        }
    ...
    global $zm_request;
        ...
        $query = "select banners_id, banners_title, banners_image, banners_html_text, banners_open_new_windows, banners_url
                  ...
                  $filter;
        $query = $this->db_->bindVars($query, ":id", $id, "integer");
        $results = $this->db_->Execute($query);
