...
class ZMThemeSupportPatch extends ZMFilePatch {
            ...
            if (is_writeable(_ZM_ZEN_INDEX_PHP)) {
                ...
                foreach ($lines as $line) {
                    ...
                    if (false !== strpos($line, "require") && false !== strpos($line, "html_header.php")) {
                        array_push($patchedLines, "  include('".ZM_ROOT."store.php'); /* added by ZenMagick installation patcher */");
                    }
                ...
                zm_log("** ZenMagick: no permission to patch theme support into index.php", ZM_LOG_ERROR);
                return false;
            }
        } else {
            // disabled
    ...
    function undo() {
        ...
        if (is_writeable(_ZM_ZEN_INDEX_PHP)) {
            ...
            foreach ($lines as $line) {
                if (false !== strpos($line, "include") && false !== strpos($line, ZM_ROOT."store.php")) {
                    continue;
                }
            ...
            zm_log("** ZenMagick: no permission to patch index.php for uninstall", ZM_LOG_ERROR);
            return false;
        }
