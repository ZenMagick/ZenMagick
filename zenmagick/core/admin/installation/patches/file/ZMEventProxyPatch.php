...
class ZMEventProxyPatch extends ZMFilePatch {
    ...
    function patch($force=false) {
        ...
        if ((zm_setting('isEnablePatching') && zm_setting('isEventProxySupport')) || $force) {
            if (is_writeable(_ZM_ZEN_BASE_PHP)) {
                ...
                zm_log("** ZenMagick: no permission to patch event proxy support into class.base.php", 1);
                return false;
            }
        } else {
            // disabled
    ...
    function undo() {
        ...
        if (is_writeable(_ZM_ZEN_BASE_PHP)) {
            ...
            zm_log("** ZenMagick: no permission to patch class.base.php for uninstall", 1);
            return false;
        }
