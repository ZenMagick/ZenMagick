...
class ZMCouponAdminMailPatch extends ZMFilePatch {
    ...
    function patch($force=false) {
        ...
        if ((ZMSettings::get('isEnablePatching')) || $force) {
            if (is_writeable(_ZM_ZEN_COUPON_ADMIN_PHP)) {
                ...
                $this->log("** ZenMagick: no permission to patch coupon admin mail fix into coupon_admin.php", ZM_LOG_ERROR);
                return false;
            }
        } else {
            // disabled
            $this->log("** ZenMagick: patch customer edit support disabled - skipping");
            return false;
        }
    ...
    function undo() {
        ...
        if (is_writeable(_ZM_ZEN_COUPON_ADMIN_PHP)) {
            ...
            $this->log("** ZenMagick: no permission to patch coupon_admin.php for uninstall", ZM_LOG_ERROR);
            return false;
        }
