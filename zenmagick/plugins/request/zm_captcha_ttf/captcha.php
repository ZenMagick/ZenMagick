 ...
 * @version $Id: captcha.php v 2.7 28.04.2007 6:12 AndrewBerezin $
 */
...
class pcaptcha {
	...
	function pcaptcha() {
		...
		$this->img_href = zen_href_link('captcha_img', zen_session_name() . '=' . zen_session_id(), 'SSL', false, false, true);
	}
 ...
 */
	function redraw_button($image, $alt = '', $parameters = '', $sec_class = '') {
		...
		return zen_image_submit($image, $alt, $redraw . ' ' . $parameters, $sec_class );
	}
 ...
 */
	function generateCaptcha() {
		...
		if($this->_gd_version['version'] >= 2) {
			...
			$imageColorFunc = 'imagecolorclosest';
		}

		$this->_background = array('r' => mt_rand($this->backgroundColor[0][0], $this->backgroundColor[1][0]),
															 'g' => mt_rand($this->backgroundColor[0][1], $this->backgroundColor[1][1]),
															 'b' => mt_rand($this->backgroundColor[0][2], $this->backgroundColor[1][2]));
		$background = $imageColorFunc($this->img, $this->_background['r'], $this->_background['g'], $this->_background['b']);

		$this->_linesColor = array('r' => mt_rand($this->linesColor[0][0], $this->linesColor[1][0]),
															 'g' => mt_rand($this->linesColor[0][1], $this->linesColor[1][1]),
															 'b' => mt_rand($this->linesColor[0][2], $this->linesColor[1][2]));
		$linesColor = $imageColorFunc($this->img, $this->_linesColor['r'], $this->_linesColor['g'], $this->_linesColor['b']);

		imagefilledrectangle($this->img, 0, 0, $this->img_width, $this->img_height, $background);
/*
		$y1 = 0;
		$y2 = $this->img_height;
...
*/
		...
		for ($i = 0; $i < $this->captchaCode_length; $i++) {
			...
			} while ($charForeColor['r'] == $this->_background['r']);
			do {
				$charForeColor['g'] = mt_rand($this->textColor[0][0], $this->textColor[1][0]);
			} while ($charForeColor['g'] == $this->_background['g']);
			do {
				$charForeColor['b'] = mt_rand($this->textColor[0][1], $this->textColor[1][1]);
			} while ($charForeColor['b'] == $this->_background['b']);
			do {
				$charBackColor['r'] = ($charForeColor['r'] < 100 ? $charForeColor['r'] * 2 : mt_rand($this->textColor[0][0], $this->textColor[1][0]));
			} while (($charBackColor['r'] == $this->_background['r']) && ($charBackColor['r'] == $charForeColor['r']));
			do {
				$charBackColor['g'] = ($charForeColor['g'] < 100 ? $charForeColor['g'] * 2 : mt_rand($this->textColor[0][1], $this->textColor[1][1]));
			} while (($charBackColor['g'] == $this->_background['g']) && ($charBackColor['g'] == $charForeColor['g']));
			do {
				$charBackColor['b'] = ($charForeColor['b'] < 100 ? $charForeColor['b'] * 2 : mt_rand($this->textColor[0][2], $this->textColor[1][2]));
			} while (($charBackColor['b'] == $this->_background['b']) && ($charBackColor['b'] == $charForeColor['b']));
			$charForeColor = $imageColorFunc($this->img, $charForeColor['r'], $charForeColor['g'], $charForeColor['b']);
			$charBackColor = $imageColorFunc($this->img, $charBackColor['r'], $charBackColor['g'], $charBackColor['b']);
			// Add the letter
...
//        imagettftext($this->img, $size, $rotation, $x+2,   $y, $charBackColor, $font, $this->captchaCode[$i]);
				...
				$size = 5;
				$charBackColor = $imageColorFunc($this->img, 0, 0, 255);
				$x = 26;
				$y = 20;
				$s = 36;
				if($this->chars_shadow) {
