...
class WikiController extends ZMController {
    ...
    function processGet($request) {
        ...
        $request->getToolbox()->crumbtrail->clear();
        $request->getToolbox()->crumbtrail->addCrumb("Wiki", zm_href('wiki', '', false));
        $page = $request->getParameter('page', 'WikiRoot');
        $page = str_replace('_', ' ', $page);
        // capitalise words
        $page = ucwords($page);
        $page = zm_l10n_get($page);
        $request->getToolbox()->crumbtrail->addCrumb(zm_l10n_get($page));
    ...
    function processPost($request) {
        ...
        $request->getToolbox()->crumbtrail->addCrumb("Wiki", zm_href('wiki', '', false));
        $page = $request->getParameter('page', 'WikiRoot');
        $request->getToolbox()->crumbtrail->addCrumb(zm_format_title($page));
