...
<?php
    ...
    function zm_catalog_catalog_tree($categories, $index=1, $level=0, $parent=0, $urlPrefix='#') {
    global $zm_products, $zm_dtree_catalog_map;
        foreach ($categories as $category) {
            ...
            echo "catalog.add(".$index++.",".$parent.",'".$category->getName().($hasProducts?"&gt;":"")."'".$url.",'','','includes/dtree/img/folder.gif','includes/dtree/img/folderopen.gif');\n";
            $zm_dtree_catalog_map[$category->getId()] = $myIndex;
            if ($category->hasChildren()) {
                $index = zm_catalog_catalog_tree($category->getChildren(), $index, $level+1, $myIndex, $urlPrefix);
            }
        }
        return $index;
    }

  function zm_catalog_dtree($pUrlPrefix='#', $cUrlPrefix='#', $folderLinks=false) {
  global $zm_categories, $zm_products, $zm_dtree_catalog_map, $zm_request;
      ...
      $index = zm_catalog_catalog_tree($zm_categories->getCategoryTree(), 1, 0, 0, $cUrlPrefix); 
