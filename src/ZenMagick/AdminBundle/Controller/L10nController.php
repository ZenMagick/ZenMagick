...
class L10nController extends DefaultController
{
    ...
    protected function processInternal($request)
    {
        ...
        $sharedMap = array();
        if ($vd['scanShared']) {
            $sharedMap = $scanner->buildL10nMap($rootDir.'/src/shared');
        }
        ...
        $adminMap = array();
        if ($vd['scanAdmin']) {
            $adminLibMap = $scanner->buildL10nMap($kernel->getRootDir().'/src/ZenMagick/AdminBundle');
        }
        ...
        if (null != $vd['themeId']) {
            ...
            $storeMap = $scanner->buildL10nMap($kernel->getRootDir().'/src/ZenMagick/StoreBundle');
