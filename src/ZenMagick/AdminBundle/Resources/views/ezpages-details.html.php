  ...
  <legend><?php _vzm('Navigation') ?></legend>
    <p>
      ...
      <label for="headerSort"><?php _vzm('Sort') ?></label>
          ...
          <?php echo Beans::getBean('booleanFormWidget#id=header&name=header&title=Header&value='.$ezPage->isHeader())->render($app->getRequest(), $templateView) ?>
      </fieldset>
      <fieldset style="float:left;width:15%;border:1px solid #aaa;padding:4px;margin:0 8px 0 0;">
      ...
      <label for="sideboxSort"><?php _vzm('Sort') ?></label>
          ...
          <?php echo Beans::getBean('booleanFormWidget#id=sidebox&name=sidebox&title=Sidebox&value='.$ezPage->isSidebox())->render($app->getRequest(), $templateView) ?>
      </fieldset>
      <fieldset style="float:left;width:15%;border:1px solid #aaa;padding:4px;margin:0 8px 0 0;">
      ...
      <label for="footerSort"><?php _vzm('Sort') ?></label>
          ...
          <?php echo Beans::getBean('booleanFormWidget#id=footer&name=footer&title=Footer&value='.$ezPage->isFooter())->render($app->getRequest(), $templateView) ?>
      </fieldset>
      ...
      <label for="tocSort"><?php _vzm('TOC Sort') ?></label>
          ...
          <?php echo Beans::getBean('booleanFormWidget#id=toc&name=toc&title=TOC&value='.$ezPage->isToc())->render($app->getRequest(), $templateView) ?>
      </fieldset>
    </p>
  </fieldset>
  ...
  <legend><?php _vzm('Link Options') ?></legend>
    ...
    <label for="newWin"><?php _vzm('Link Target') ?></label>
      <?php
          ...
          echo $select->render($app->getRequest(), $templateView);
      ?>
      <?php echo Beans::getBean('booleanFormWidget#id=SSL&name=SSL&title='._zm('Secure Link').'&value='.$ezPage->isSsl())->render($app->getRequest(), $templateView) ?>
    </p>
    <p>
