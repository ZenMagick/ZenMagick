...
class ToolboxMacro extends ToolboxTool
{
     ...
     *
     * @param array categories An <code>array</code> of <code>ZenMagick\StoreBundle\Entity\Category</code> instances.
     ...
     * </pre></code>
     *
     * @param ZenMagick\StoreBundle\Entity\Product product A <code>Product</code> instance.
    ...
    protected function productTextAttribute($product, $attribute)
    {
        ...
        foreach ($attribute->getValues() as $value) {
            $id = 'id_'.$attribute->getId().'_'.$index++;
            $name = 'id['.$settingsService->get('textOptionPrefix').$attribute->getId().']';
    ...
    protected function productUploadAttribute($product, $attribute, $uploadIndex)
    {
        ...
        foreach ($attribute->getValues() as $value) {
            ...
            $text .= '<input type="hidden" name="'.$settingsService->get('uploadOptionPrefix').$uploadIndex.'" value="'.$attribute->getId().'" />';
            $text .= '<input type="hidden" name="'.$settingsService->get('textOptionPrefix').$settingsService->get('uploadOptionPrefix').$uploadIndex.'" value="" />';
            array_push($elements, $text);
        }
        $element['html'] = $elements;
